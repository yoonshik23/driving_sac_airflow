...
from .utils import 구분데이터붙이기, generate_polygon_random_points, 공휴일출력, 예약가능시간출력, 이웃시간판단, time_to_minutes, which_시간대
...
from sqlalchemy import text, LargeBinary, Integer
...
class Simulator_01():
    ...
    def __init__(self, simulation_start_dt, simulation_end_dt, db_info, is_first = False, data_start_date = None, data_end_date = None, weight_학생숫자 = 1, 차량숫자 = 12, 시군구_ids = ['11220', '11230', '11240'], mode = 'base', try_num = 0):
        ...
        if is_first == True:
            ...
            # data_start_dt = self.data.sort_values('예약희망일시').reset_index(drop=True).loc[0, '예약희망일시']
            # data_end_dt = pd.to_datetime(data_end_date)
            # insert_query = text('''INSERT INTO datamart."history_학습id" ("train_data_start_dt", "train_data_end_dt") VALUES (:st, :ed) RETURNING "train_id";''')
            
        #     with self.db_handler.engine.connect() as connection:
        #         trans = connection.begin()
        #         result = connection.execute(insert_query, {'st': data_start_dt, 'ed': data_end_dt})
                
        #         # 생성된 id 값을 가져옴
        #         self.train_id = result.scalar()
        #         trans.commit()
        ...
        #     self._클러스터링결과저장()
            ...
            강남3구_집계구_경계_중심 = gpd.read_postgis(query, self.db_handler.engine, geom_col='중심')
            
            self.위치데이터 = 강남3구_집계구_경계_중심.loc[:, ['집계구_id', '수요인구', '중심']]
            # self.위치데이터 = self.위치데이터.set_geometry('경계')
            # self.위치데이터['경계'] = self.위치데이터['경계'].to_crs(epsg = 3857)
            self.위치데이터['중심'] = self.위치데이터['중심'].to_crs(epsg = 3857)
            # 위치별 30분 이내거리(지도직선거리 5키로) index
            self.위치_5000이내 = {}
            for i in range(len(self.위치데이터)):
                self.위치_5000이내[i] = []
                for j in range(i+1, len(self.위치데이터)):
                    if self.위치데이터.loc[i, '중심'].distance(self.위치데이터.loc[j, '중심']) <= 5000:
            ...
            query = f"""
            SELECT *
            FROM datamart.차량정보기록_시군구_뷰
            WHERE 시군구_id IN ({', '.join(f"'{id}'" for id in 시군구_ids)})
            """
            # 쿼리 실행 및 GeoDataFrame으로 변환
            self.차량데이터 = gpd.read_postgis(query, self.db_handler.engine, geom_col='주차_point')
            # self.차량데이터 = self.db_handler.select(table = '차량정보기록', schema='datamart', columns = ['차량코드', '차종', '기록일시', '주차_point'],
            #                                    gis_col = '주차_point', 
            #                                   )
            self.차량데이터 = self.차량데이터.set_geometry('주차_point')
        ...
        if 차량숫자 <= 12:
            ...
            for _ in range(차량숫자-12):
                random_index = np.random.choice(self.차량데이터.index)
                random_row = self.차량데이터.loc[[random_index]]
                self.차량데이터 = gpd.GeoDataFrame(pd.concat([self.차량데이터, random_row], ignore_index=True))
                # self.차량데이터 = self.차량데이터.append(random_row, ignore_index=True)
                self.차량데이터 = self.차량데이터.reset_index(drop = True)
                self.차량데이터.iloc[12+_, 0] = str(12+_)
        ...
        self.종료일 = pd.to_datetime(simulation_end_dt).tz_convert('Asia/Seoul')
    ...
    def run(self, 선택최적 = 0, 선택rule = 'rule1'):
        ...
        self.count_total_차량이동거리 = 0
        self.count_오늘_차량이동거리 = 0
        self.count_total_차량당이동거리 = 0
        self.count_오늘_차량당이동거리 = 0
        self.count_total_모예_수업수 = 0
        self.count_오늘_모예_수업수 = 0
    ...
    def reset(self, 선택최적 = 0, 선택rule = 'rule1', mode = 'base'):
        '''
        self.남은예약자s 에서 한 명씩 불러오며 진행.
        1. 차량현황, 예약현황, 예약자 위치를 기반으로 예약 가능 목록 보여주기
        2. 예약 희망일시와 비교해 예약 여부 결정 (여러 개 있으면 아예 랜덤?)
        ...
        self.모든예약 = {'예약_일시': [], '예약_point': [], '예약_point_idx': [], '예약_차종': [], '연월일': []}
    ...
    def step(self, actions, is_reinforce = True):
        ...
        if self.simulation_done == False:
            st = time.time()
            reward[0] = self._예약선택(self.예약자, actions, is_reinforce)
            ...
            if len(self.남은예약자s) == 0:
                ...
                self._통계값산출()
                next_state = [[[0]*31, [[] for _ in range(25)]] for i in range(30)]
            else:
                self.예약자 = self.남은예약자s.pop(0)
                self.past_예약자s.append(self.예약자)
                ...
                for i in range(며칠뒤):
                    ...
                    continuous_sate = [tmp_시차.total_seconds(), self.예약자['예약희망_point_idx'].item(), self.위치데이터.loc[self.예약자['예약희망_point_idx'], '수요인구'].item(), bool(tmp.loc[0, '휴일']), tmp['mean'].mean().item(), tmp['std'].mean().item()]
            ...
            if dones is not True:
                pass
            self.모델_응답속도.append(time.time()-st)
        else:
            dones = [1]*며칠뒤
            next_state = [[[0]*31, [[] for _ in range(25)]] for i in range(30)]
            pass
    ...
    def make_init_states(self, sample_num = 100):
        ...
        for i in range(sample_num):
            ...
            continuous_sate = [tmp_시, tmp.index[i].item(), tmp['수요인구'].iloc[i].item(), bool(tmp_df.loc[0, '휴일']), tmp_df['mean'].mean().item(), tmp_df['std'].mean().item()] + [0]*25
    ...
    def _통계값산출(self):
        ...
        if self.count_오늘_수업수 != 0:
            ...
            if self.mode == 'offline_optimal':
                print("{:<15}".format('하루 최적 차량당 수업횟수')+': ' "{:^6}".format(round((self.count_오늘_모예_수업수/self.count_모예_오늘_차량수), 2)), "{:<15}".format('total 최적 차량당 수업횟수')+': ' "{:^6}".format(round((self.count_total_모예_수업수/self.count_모예_total_차량수), 2)))
            print("{:<15}".format('하루 차량당 이동거리(km)')+': ' "{:^6}".format(self.count_오늘_차량당이동거리/1000), "{:<15}".format('total 차량당 이동거리(km)')+': ' "{:^6}".format(self.count_total_차량당이동거리/1000))

            self.일자별_최적_수업수[self.일자별예약고객수['일자'][-1]] = self.count_오늘_모예_수업수
            self.일자별_최적_가동차량수[self.일자별예약고객수['일자'][-1]] = self.count_모예_오늘_차량수
            self.일자별_수업수[self.일자별예약고객수['일자'][-1]] = self.count_오늘_수업수
            self.일자별_가동차량수[self.일자별예약고객수['일자'][-1]] = self.count_오늘_차량수
            self.일자별_차량당_이동거리[self.일자별예약고객수['일자'][-1]] = self.count_오늘_차량당이동거리
    ...
    def _예약선택(self, 예약자, actions, is_reinforce = True):
            ...
            try:
                ...
                if  is_reinforce == True:
                    if (actions[(희망일시.date() - self.오늘.date()).days - 1][0][시간_인덱스_딕셔너리[희망일시.strftime('%H_%M')]] < 0):
                        ...
                        if self.mode == 'offline_optimal':
                            self.모든예약['예약_일시'].append(희망일시)
                            self.모든예약['예약_point'].append(예약자['예약희망_point'])
                            self.모든예약['예약_point_idx'].append(예약자['예약희망_point_idx'])
                            self.모든예약['예약_차종'].append('2종보통')
                            self.모든예약['연월일'].append(희망일시.strftime('%Y%m%d'))
                        continue
                ...
                if self.일자별최적화시행수[희망일시.strftime('%Y%m%d_%H_%M')] == self.선택최적:
                    ...
                    if status == cp_model.FEASIBLE or status == cp_model.OPTIMAL:
                        ...
                        예약일s.append([희망일시, self.차량데이터.loc[solver.Value(vehicle_assignments[-1]), '차량코드'], 1, [self.차량데이터.loc[solver.Value(vehicle_assignments[i]), '차량코드'] for i in range(num_customers-1)], customer_loc_idxes[:-1], [x for x in tmp_남예['예약_일시']]])
        ...
        if len(예약일s) > 0:
            ...
            if self.mode == 'offline_optimal':
                self.모든예약['예약_일시'].append(예약일)
                self.모든예약['예약_point'].append(예약자['예약희망_point'])
                self.모든예약['예약_point_idx'].append(예약자['예약희망_point_idx'])
                self.모든예약['예약_차종'].append('2종보통')
                self.모든예약['연월일'].append(예약일.strftime('%Y%m%d'))    
            ...
            if 최적화여부 == 1:
                ...
                for column in tmp_append:
                    self.위치별예약가능일시[column] = [self.차량데이터['차량코드'].to_list()]*len(self.위치데이터)
    ...
    def _차량최적화(self):
        ...
        if len(예약s) > 0 :
            ...
            if status == cp_model.FEASIBLE or status == cp_model.OPTIMAL:
                ...
                # print(idxes)
                for i in range(num_customers):
                    tmp_차량코드 = self.차량데이터.loc[solver.Value(vehicle_assignments[i]), '차량코드']
                    self.남은예약['임시배정차량코드'][idxes[i]] = tmp_차량코드
            ...
            for car_id in 예약s['임시배정차량코드'].unique():
                tmp_예약s = 예약s[예약s['임시배정차량코드'] == car_id]
                tmp_예약s = tmp_예약s.sort_values(by='예약_일시')
                
                # self.count_오늘_차량이동거리 += self.차량데이터.loc[self.차량데이터['차량코드'] == car_id, '주차_point'].iloc[0].distance(tmp_예약s.iloc[0, 1])
                for i in range(1, len(tmp_예약s)):
                    self.count_오늘_차량이동거리 += tmp_예약s.iloc[i-1, 1].distance(tmp_예약s.iloc[i, 1])
            self.count_total_차량이동거리 += self.count_오늘_차량이동거리
            self.count_오늘_차량당이동거리 = self.count_오늘_차량이동거리/self.count_오늘_차량수
            self.count_total_차량당이동거리 = self.count_total_차량이동거리/self.count_total_차량수

            
            
                    
            if self.mode == 'offline_optimal':
                tmp_모예 = pd.DataFrame(self.모든예약)
                모예약s = tmp_모예[tmp_모예['연월일']==self.오늘.strftime('%Y%m%d')].reset_index(drop = True)
                ...
                if len(모예약s) > 0 :
                    ...
                    # 각 고객이 배정된 차량이 실제로 사용되었는지 여부
                    for i in range(num_customers):
                        모예_model.AddElement(모예_vehicle_assignments[i], 모예_vehicle_used, 1)
                    ...
                    if 모예_status == cp_model.FEASIBLE or 모예_status == cp_model.OPTIMAL:
                        condition = lambda x: x == self.오늘.strftime('%Y%m%d')
                        idxes = [index for index, value in enumerate(self.남은예약['연월일']) if condition(value)]
                        # print(idxes)
                        tmp_차량코드s = []
                        for i in range(num_customers):
                            if 모예_solver.Value(모예_vehicle_assignments[i]) != num_vehicles:  # 유효한 배정만 고려
                                tmp_차량코드s.append(self.차량데이터.loc[모예_solver.Value(모예_vehicle_assignments[i]), '차량코드'])
                        tmp = []
                        for i in range(num_customers):
                            tmp.append(모예_solver.Value(모예_customer_assigned[i]))

                        self.count_모예_오늘_차량이동거리 = 0
                        self.count_모예_오늘_차량수 = len(pd.Series(tmp_차량코드s).unique())
                        self.count_모예_total_차량수 += self.count_모예_오늘_차량수
                        self.count_오늘_모예_수업수 = sum(tmp)
                        self.count_total_모예_수업수 += self.count_오늘_모예_수업수
                    
                    else:
                        print('뭔가잘못됨. 받을 수 있는 고객만 받게 했는데 FEASIBLE SOLUTION이 없는 말이 안되는 상황.')
    ...
    def _01_수요데이터불러오기(self, start_datetime : datetime.datetime = None, end_datetime = None, 시군구_ids = ['11220', '11230', '11240']):
        '''
        ...
        if start_datetime is not None:
            wheres.append('"예약희망일시" >= \'{st_dt}\''.format(
                st_dt = start_datetime.strftime('%Y-%m-%d %H:%M:%S%z')
            ))
        if end_datetime is not None:
            wheres.append('"예약희망일시" <= \'{ed_dt}\''.format(
                ed_dt = end_datetime.strftime('%Y-%m-%d %H:%M:%S%z')
            ))
        if 시군구_ids is not None:
            tmp = ' OR '.join(['"시군구_id" = \'{}\''.format(시군구) for 시군구 in 시군구_ids])
            tmp = '(' + tmp + ')' 
            wheres.append(tmp)
        self.data = self.db_handler.select(table = '고객수요_집계구_뷰', schema='datamart', columns = ['고객_접속일시', '예약희망일시', '예약희망_point'], wheres = wheres, orderbys = ['"예약희망일시" asc'], 
                                           gis_col = '예약희망_point', 
                                          )
        # print(self.data)
        "데이터 못불러오면 아래 코드에서 에러남."
    ...
    def _02_구분데이터붙이기(self):
        ...
        '''
        self.data = 구분데이터붙이기(self.data.copy(), '예약희망일시')
    ...
    def _03_클러스터링(self):
        ...
        self.cluster._02_cutting(method = 'threshold', values = {'threshold': 0.9})
    ...
    def _04_학생_생성(self, dt_start, dt_end, 위치_데이터, weight_학생숫자 = 1):
        ...
        choices = random.choices(np.arange(0, len(위치_데이터)), weights = 위치_데이터['수요인구'], k=len(self.예약자s)) # 인구수에 비례하게 고객 위치 생성
        ...
        for idx_집계구 in choices:
            ...
            points.append(위치_데이터.loc[idx_집계구, '중심'])
    ...
    def _클러스터링결과저장(self):
        ...
        pickled_object = pickle.dumps(클러스터링결과)
        tmp_df = pd.DataFrame({
            'train_id': [self.train_id],
            'demand_cluster_pickle': [pickled_object]
        })
        tmp_df.to_sql('storage_demand_cluster', self.db_handler.engine, schema = 'datamart', index = False, if_exists='append', dtype={'train_id': Integer, 'demand_cluster_pickle': LargeBinary})
    ...
    def _클러스터링결과불러오기(self):
        ...
        #     # 저장된 데이터를 SELECT하여 불러옴
        #     result = pd.read_sql('SELECT * FROM example_table', connection)
        
        # # 불러온 데이터를 피클로 언피클
        # unpickled_object = pickle.loads(result['data'][0])
    ...
    def __init__(self, value_kinds, 모든키, 모든원소):
        ...
        for i in range(len(tmp_keys)):
            for j in range(i+1, len(tmp_keys)):
                ...
                self.군집간유사도[str(sorted([tmp_keys[i], tmp_keys[j]]))] = (stat, p_value)
        self.history = []
    def _01_군집화(self):
        '''
        거리척도 우선 from scipy.stats import ks_2samp 기준.
        ...
        while len(self.군집s) > 1:
            # p value가 가장 큰 key
            max_key = max(self.군집간유사도, key=lambda k: self.군집간유사도[k][1])
            # print(max_key)
            max_value = self.군집간유사도[max_key]
            ...
            for key in tmp_keys:
                ...
                self.군집간유사도[str(sorted([new_key, key]))] = (stat, p_value)

                self.군집간유사도.pop(str(sorted([key_a, key])))
                self.군집간유사도.pop(str(sorted([key_b, key])))
            self.군집간유사도.pop(str(sorted([key_b, key_a])))
