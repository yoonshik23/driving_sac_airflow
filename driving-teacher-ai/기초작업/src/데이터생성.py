...
class 시뮬레이션_고객수요():
    ...
    def __init__(self, 
                 ...
                 위치_데이터:pd.DataFrame,
                 총일수:int=365, 
                 휴일_시간별_예약_평균:list=[6, 6, 10, 10, 10, 10, 10, 10, 10, 10, 10, 8, 6], # 116
                 비수기_업무일_시간별_예약_평균:list=[1, 1, 2, 2, 2, 2, 2, 2, 2, 10, 10, 8, 6], # 50
                 성수기_업무일_시간별_예약_평균:list=[3, 3, 6, 6, 6, 6, 6, 6, 6, 10, 10, 8, 6], # 82
        ...
        self.고객접속기록 = pd.DataFrame(columns = ['고객_코드', '고객_접속일시', '고객_접속point', '예약희망일시', '예약희망_point', '예약희망_차종'])
...
class 시뮬레이션_고객접속기록():
    ...
    def __init__(self, 시작일:str, 
                 위치_데이터:pd.DataFrame,
                 총일수:int=365, 
                 휴일_시간별_예약_평균:list=[12, 20, 20, 20, 20, 20, 20],
                 업무일_시간별_예약_평균:list=[5, 5, 5, 20, 20],
                예약전환률 = 0.2):
        '''
        시작일:str, 
        위치_데이터:pd.DataFrame centroid 컬럼 필수
        총일수:int=365, 
        휴일_시간별_예약_평균:list=[20, 20, 20, 20, 20, 20], 
        업무일_시간별_예약_평균:list=[5, 5, 5, 20, 20], 
        '''
        self.위치_데이터 = 위치_데이터.reset_index(drop=True)
        self.시작일 = pd.to_datetime(시작일)
        ...
        self.총일수 = 총일수
        self.휴일_시간별_예약_평균 = 휴일_시간별_예약_평균
        self.업무일_시간별_예약_평균 = 업무일_시간별_예약_평균
        self.예약전환률 = 예약전환률
        self.engine = create_engine("postgresql://postgres:postgres@localhost:5432/postgres")

        self.고객접속기록 = pd.DataFrame(columns = ['고객_코드', '고객_접속일시', '고객_접속point', '예약_유무', '예약_일시', '예약_point', '예약_차종'])
    def 데이터생성시뮬실행(self):
        고객_코드0 = 0
        for i in range(self.총일수):
    ...
    def _02_오늘예약횟수생성(self): 
        if self.오늘휴일 is True:
            ...
            시간별_예약_평균 = self.업무일_시간별_예약_평균

        self.오늘_시간별_예약_횟수 = list(np.random.poisson(lam = 시간별_예약_평균))
        ...
        for i, 횟수 in enumerate(self.오늘_시간별_예약_횟수) :
            for j in range(횟수):
                ...
                예약_일시 = pd.to_datetime(예약_일시)
                self.오늘예약기록.loc[k, '고객_접속일시'] = 예약_일시 - pd.Timedelta(value = np.random.randint(low = 144, high = 10080), unit = 'minute')  # 하루 ~ 1주일 전에 접속하여 예약
                self.오늘예약기록.loc[k, '예약_유무'] = True
                self.오늘예약기록.loc[k, '예약_일시'] = pd.to_datetime(예약_일시)
                

                k += 1
            
    def _03_더미접속생성(self):
        기준_접속일시 = self.오늘.strftime("%Y-%m-%d")+' 00:00:00'
        더미접속갯수 = round(len(self.오늘예약기록)*(1-self.예약전환률)/self.예약전환률)
        값_접속일시 = np.random.randint(low = 144, high = 10080, size = 더미접속갯수)
        접속일시s = []
        for 값 in 값_접속일시:
            접속일시s.append(pd.to_datetime(기준_접속일시) - pd.Timedelta(value = 값, unit = 'minute'))  # 기준보다 하루 ~ 1주일 전에 접속
        
        더미접속기록 = pd.DataFrame()
        더미접속기록['고객_접속일시'] = 접속일시s
        더미접속기록['예약_유무'] = False

        self.오늘예약기록 = pd.concat([self.오늘예약기록, 더미접속기록]).reset_index(drop=True)

    def _04_장소생성(self):
        # idx_경계구s = np.random.randint(low = 0, high = , size = len(self.오늘예약기록))
        choices = random.choices(np.arange(0, len(self.위치_데이터)), weights = self.위치_데이터['count'], k=len(self.오늘예약기록)) # 인구수에 비례하게 고객 위치 생성

        points = []
        for idx_집계구 in choices:
            points.append(self.위치_데이터.loc[idx_집계구, 'centroid'])
        self.오늘예약기록['고객_접속point'] = points
        self.오늘예약기록['고객_접속point'] = self.오늘예약기록['고객_접속point'].apply(lambda x: WKTElement(x.wkt, srid=4326))

        self.오늘예약기록.loc[self.오늘예약기록['예약_유무']==True, '예약_point'] = self.오늘예약기록.loc[self.오늘예약기록['예약_유무']==True, '고객_접속point']

    def _05_디비적재(self):
        self.오늘예약기록.sort_values('고객_접속일시', ascending = True, inplace = True)
        self.오늘예약기록.to_sql(name = '고객접속기록',
                                con = self.engine,
                                schema = 'datamart',
                                if_exists = 'append',
                                index = False, dtype={'고객_접속point': Geometry('POINT', srid=4326),
                                                     '예약_point': Geometry('POINT', srid=4326)})
