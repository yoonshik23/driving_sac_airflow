            ...
            try:
                ...
                if self.일자별최적화시행수[희망일시.strftime('%Y%m%d_%H_%M')] == self.선택최적:
                    ...
                    if status == cp_model.FEASIBLE or status == cp_model.OPTIMAL:
                        ...
                        예약일s.append([희망일시, self.차량데이터.iloc[solver.Value(vehicle_assignments[-1]), 0], 1, [self.차량데이터.iloc[solver.Value(vehicle_assignments[i]), 0] for i in range(num_customers-1)], customer_loc_idxes[:-1], [x for x in tmp_남예['예약_일시']]])
    ...
    def _차량최적화(self):
        ...
        if len(예약s) > 0 :
            ...
            if status == cp_model.FEASIBLE or status == cp_model.OPTIMAL:
                ...
                # print(idxes)
                for i in range(num_customers):
                    tmp_차량코드 = self.차량데이터.iloc[solver.Value(vehicle_assignments[i]), 0]
                    self.남은예약['임시배정차량코드'][idxes[i]] = tmp_차량코드
